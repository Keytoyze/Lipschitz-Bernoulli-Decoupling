...
class ClickSimulationFeed(BaseInputFeed):
    ...
    def prepare_sim_clicks_with_index(
        ...
        doc_list = list([-1 if data_set.initial_list[i][x]
                               < 0 else base + x for x in range(self.rank_list_size)])
        feature_list = list([[0] * len(data_set.features[0]) if data_set.initial_list[i][x] < 0
                             else data_set.features[data_set.initial_list[i][x]]
    ...
    def get_batch(self, data_set, check_validation=False):
        ...
        for l in range(self.rank_list_size, self.model.max_candidate_num):
            input_feed["docid_input" + str(l) + ":0"] = np.zeros_like(batch_docid_inputs[0])
            input_feed["label" + str(l) + ":0"] = np.zeros_like(batch_labels[0])
