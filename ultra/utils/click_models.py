...
def loadModelFromJson(model_desc):
    if model_desc['model_name'] == 'position_biased_model':
        click_model = PositionBiasedModel()
    elif model_desc['model_name'] == 'user_browsing_model':
    ...
    elif model_desc['model_name'] == 'individual_ips_model':
        click_model = IndividualIpsModel()
    elif model_desc['model_name'] == 'click_matrix_model':
        click_model = ClickMatrixModel()
    else:
        raise ValueError("Unknown click model: %s" % model_desc['model_name'])
    click_model.eta = model_desc['eta']
    click_model.click_prob = model_desc['click_prob']
    click_model.exam_prob = model_desc['exam_prob']
    if 'weight' in model_desc:
        click_model.weight = model_desc['weight']
    if 'importance_idx' in model_desc:
        click_model.importance_idx = model_desc['importance_idx']
    if 'ep_1_neg' in model_desc:
        click_model.ep_1_neg = model_desc['ep_1_neg']
    if 'click_matrix' in model_desc:
        click_model.click_matrix = np.asarray(model_desc['click_matrix'])
    return click_model
...
class IndividualIpsModel(ClickModel):

    @property
    def model_name(self):
        return 'individual_ips_model'
    ...
    def sampleClicksForOneList(self, label_list, doc_list):
        click_list, exam_p_list, click_p_list = [], [], []
        for rank in range(len(label_list)):
            click, exam_p, click_p = self.sampleClick(rank, label_list[rank], doc_list[rank])
            click_list.append(click)
            exam_p_list.append(exam_p)
            click_p_list.append(click_p)
        return click_list, exam_p_list, click_p_list
    ...
    def sampleClick(self, rank, relevance_label, feature):
        if not relevance_label == int(relevance_label):
            print('RELEVANCE LABEL MUST BE INTEGER!')
        relevance_label = int(relevance_label) if relevance_label > 0 else 0
        exam_p = self.getExamProb(rank, feature, relevance_label)
        click_p = self.click_prob[relevance_label if relevance_label < len(
            self.click_prob) else -1]
        click = 1 if random.random() < exam_p * click_p else 0
        return click, exam_p, click_p

    def getExamProb(self, rank, feature, label):
        importance_eta = 0
        for i in range(len(self.importance_idx)):
            importance_eta += feature[self.importance_idx[i]] * self.weight[i]
        examine_p = self.exam_prob[rank if rank < len(self.exam_prob) else -1]
        importance_eta = importance_eta + 1
        importance_eta = max(importance_eta, 0)
        return examine_p ** importance_eta
...
class TrustBiasedModel(ClickModel):
    ...
    def getClickProb(self, rank, label):
        ...
        ep_minus = self.ep_1_neg / (rank + 1)
        return primitive_click_prob * ep_plus + (1 - primitive_click_prob) * ep_minus
...
class ClickMatrixModel(ClickModel):
    ...
    def sampleClicksForOneList(self, label_list, doc_list=None):
        click_list, exam_p_list, click_p_list = [], [], []
        # print(label_list)
        for rank in range(len(label_list)):
            relevance_label = label_list[rank]
            relevance_label = int(relevance_label) if relevance_label > 0 else 0
            click_p = self.click_matrix[relevance_label, rank]
